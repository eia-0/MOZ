<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            {{-- Компактная информационная панель магазина --}}
            ...
            {{-- Дополнительная информация (доставка, минимальный заказ) --}}
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-8">
                <p class="text-gray-700">{{ $store->description }}</p>
                <p class="text-sm text-gray-500 mt-2">
                    Минимальный заказ: {{ $store->min_order }} руб. |
                    Доставка: {{ $store->delivery_fee }} руб.
                </p>
                @if($store->free_delivery_from)
                    <p class="text-sm text-gray-500 mt-1">
                        Бесплатная доставка от {{ $store->free_delivery_from }} руб.
                    </p>
                @endif
            </div>
