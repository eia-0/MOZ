        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    {{-- Общая ссылка Каталог (для всех, кроме курьера) --}}
                    ...
                    @auth
                        ...
                        @elseif(auth()->user()->isCustomer())
                            ...
                            <x-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">
                                ...
                                @if($cartCount > 0)
                                    <span class="inline-flex items-center justify-center w-5 h-5 ms-1 text-xs font-bold text-white bg-red-500 rounded-full">
                                        {{ $cartCount }}
                                    </span>
                                @endif
                            </x-nav-link>
                            <x-nav-link :href="route('customer.orders')" :active="request()->routeIs('customer.*')">
                                Мои заказы
                            </x-nav-link>
                        @endif
                    @endauth
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if(auth()->user()->isStore())
                    ...
                    <x-responsive-nav-link :href="route('store.settings')" :active="request()->routeIs('store.settings')">
                        Настройки
                    </x-responsive-nav-link>
                @elseif(auth()->user()->isCourier())
                    <x-responsive-nav-link :href="route('courier.orders')" :active="request()->routeIs('courier.*')">
                        Заказы на доставку
                    </x-responsive-nav-link>
                @elseif(auth()->user()->isCustomer())
                    ...
                    <x-responsive-nav-link :href="route('customer.orders')" :active="request()->routeIs('customer.*')">
                        Мои заказы
                    </x-responsive-nav-link>
                @endif
            @endauth
